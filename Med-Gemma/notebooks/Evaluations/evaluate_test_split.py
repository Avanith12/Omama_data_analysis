...
# ---------- paths ----------
DATA_DIR = "/hpcstor6/scratch01/a/a.kanamarlapudi001/med-gemma/hf_test_messages_metadata"
BASE_ID  = "google/medgemma-4b-it"   # baseline (gated—needs token/login)
FINETUNE_DIR = "/hpcstor6/scratch01/a/a.kanamarlapudi001/med-gemma/runs/medgemma_balanced_lora/checkpoint-581"
DEBUG_LOG_FILE = "/hpcstor6/scratch01/a/a.kanamarlapudi001/med-gemma/runs/final_test_debug.jsonl"

# keep HF caches off home (optional but recommended)
os.environ.setdefault("HF_HOME", "/hpcstor6/scratch01/a/a.kanamarlapudi001/med-gemma/hf_cache")
os.environ.setdefault("HUGGINGFACE_HUB_CACHE", "/hpcstor6/scratch01/a/a.kanamarlapudi001/med-gemma/hf_cache/hub")
os.environ.setdefault("HF_DATASETS_CACHE", "/hpcstor6/scratch01/a/a.kanamarlapudi001/med-gemma/hf_cache/datasets")
os.environ.setdefault("TRANSFORMERS_CACHE", "/hpcstor6/scratch01/a/a.kanamarlapudi001/med-gemma/hf_cache/transformers")

# ---------- load data ----------
# Load either a DatasetDict (with 'validation') or a plain Dataset
data = load_from_disk(DATA_DIR)
if isinstance(data, dict):  # DatasetDict-like (train/validation/etc.)
    test_data = data["validation"]
else:
    test_data = data  # Single split Dataset saved directly

# Detect classes (should be ['NonCancer', 'Cancer'])
CLASS_NAMES = test_data.features["label"].names
LABEL_IDX = {name.lower(): idx for idx, name in enumerate(CLASS_NAMES)}

# tolerate different cancer label spellings
NONCANCER_ID = LABEL_IDX.get("noncancer")
CANCER_ID = LABEL_IDX.get("cancer", LABEL_IDX.get("indexcancer"))
if NONCANCER_ID is None or CANCER_ID is None:
    raise ValueError(f"Could not identify NonCancer/Cancer labels in dataset features: {CLASS_NAMES}")
...
def compute_metrics(preds: List[int], verbose=True):
    ...
    valid_preds = [p if p in (NONCANCER_ID, CANCER_ID) else -1 for p in preds]
    ...
    # Confusion matrix (order: NonCancer, Cancer)
    cm = confusion_matrix(filtered_refs, filtered_preds, labels=[NONCANCER_ID, CANCER_ID])
    tn, fp, fn, tp = cm.ravel()
    
    # Per-class metrics
    precision, recall, f1_per_class, support = precision_recall_fscore_support(
        filtered_refs, filtered_preds, labels=[NONCANCER_ID, CANCER_ID], zero_division=0
    )
...
def postprocess_text_to_label_lenient(text: str, true_label: int = None, debug_log: list = None) -> Tuple[int, str]:
    ...
    def log(pred_idx: int, reason: str):
    ...
    for pattern in negation_patterns:
        if re.search(pattern, t):
            return log(NONCANCER_ID, f"negation_detected: {pattern}")
    
    # Priority 4: Check for substring "noncancer" (must come before general "cancer")
    if "noncancer" in t:
        return log(NONCANCER_ID, "substring_noncancer")
...
def analyze_raw_outputs(raw_outputs, predictions, references, n_samples=10):
    ...
    for category, samples in categories.items():
        ...
        for i, (idx, raw, pred, ref) in enumerate(samples[:n_samples]):
            ...
            pred_label = CLASS_NAMES[pred] if pred in (NONCANCER_ID, CANCER_ID) else "INVALID"
...
print("="*60)
peft_cfg = PeftConfig.from_pretrained(FINETUNE_DIR)
processor_ft = AutoProcessor.from_pretrained(peft_cfg.base_model_name_or_path)
...
ft = PeftModel.from_pretrained(ft_base, FINETUNE_DIR).to("cuda").eval()
...
results_file = os.path.join(
    ...
    f"final_test_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
)
